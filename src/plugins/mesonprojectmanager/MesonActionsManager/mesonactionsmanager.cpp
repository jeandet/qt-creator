    ...
    , configureActionContextMenu(tr("Configure"))
{
    ...
    Core::ActionContainer *mbuild = Core::ActionManager::actionContainer(
...

    {
        ...
        connect(&buildTargetContextAction, &Utils::ParameterAction::triggered, this, [] {
            if (auto bs = qobject_cast<MesonBuildSystem *>(ProjectExplorer::ProjectTree::currentBuildSystem())) {
                auto targetNode = dynamic_cast<MesonTargetNode *>(ProjectExplorer::ProjectTree::currentNode());
                targetNode->build();
            }
        });
    }
...
{
    auto targetNode = dynamic_cast<const MesonTargetNode *>(ProjectExplorer::ProjectTree::currentNode());
