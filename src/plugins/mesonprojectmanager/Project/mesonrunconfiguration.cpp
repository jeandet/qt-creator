    ...
    :ProjectExplorer::RunConfiguration{target,id}
{
    ...
    connect(libAspect, &ProjectExplorer::UseLibraryPathsAspect::changed,
            envAspect, &ProjectExplorer::EnvironmentAspect::environmentChanged);

    if (Utils::HostOsInfo::isMacHost()) {
        auto dyldAspect = addAspect<ProjectExplorer::UseDyldSuffixAspect>();
        connect(dyldAspect, &ProjectExplorer::UseLibraryPathsAspect::changed,
                envAspect, &ProjectExplorer::EnvironmentAspect::environmentChanged);
    ...
    connect(target, &ProjectExplorer::Target::buildSystemUpdated, this, &ProjectExplorer::RunConfiguration::update);
}
...
void MesonRunConfiguration::updateTargetInformation()
{
    ...
    aspect<ProjectExplorer::WorkingDirectoryAspect>()->setDefaultWorkingDirectory(bti.workingDirectory);
