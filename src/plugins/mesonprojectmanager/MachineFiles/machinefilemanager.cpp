...
MachineFileManager::MachineFileManager()
{
    connect(ProjectExplorer::KitManager::instance(),
            &ProjectExplorer::KitManager::kitAdded,
            this,
            &MachineFileManager::addMachineFile);
    connect(ProjectExplorer::KitManager::instance(),
            &ProjectExplorer::KitManager::kitUpdated,
            this,
            &MachineFileManager::updateMachineFile);
    connect(ProjectExplorer::KitManager::instance(),
            &ProjectExplorer::KitManager::kitRemoved,
            this,
...
Utils::FilePath MesonProjectManager::Internal::MachineFileManager::machineFile(
...
void MachineFileManager::cleanupMachineFiles()
{
    ...
    std::for_each(std::cbegin(kits),
                  std::cend(kits),
                  [&expected, &machineFiles, this](const ProjectExplorer::Kit *kit) {
                      QString fname = machineFile(kit).toString();
                      expected.push_back(fname);
                      if (!machineFiles.contains(fname))
                          addMachineFile(kit);
                  });
    std::for_each(std::cbegin(machineFiles),
                  std::cend(machineFiles),
                  [&expected](const QString &file) {
